...
| 6 | **DC-66** — multi-commit queuing | **PROPOSED 2026-08-02** on the owner's queuing decision. Unblocks DC-57; makes NFR-PERF-02/03 meetable. Scope warning: block format and `seal` already support N, so the cap is three guards — the increment is **crash recovery, `verify`, and ordering** | owner acceptance | not yet written — RFC must be accepted first |
