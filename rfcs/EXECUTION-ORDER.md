...
| 15 | **DC-75** — merge block lineage / structural merge record | **ACCEPTED 2026-08-08.** Discharges DC-74's release condition. Gate is `block_state.rs:13-26` rejecting `BlockKind::Merge` outright — greenfield write-side design. Carries one open design question: mainline-authoritative vs both-parents-verified | **Nothing — fully unblocked 2026-08-08** when DC-74 merged at `3464e2a`. §4 still precedes design, as a design gate **§5 blocked 2026-08-08** — mainline-only ancestry traversal (architect ruling on investigation item 6) makes a second merge between the same two branches impossible and would make `verify` flag valid history; reachability and state derivation must be separated | `handoffs/DC-75-merge-block-lineage/implementation-handoff-v1.md` + addenda 1-3 |
...
| 17 | **DC-77** — docs Mermaid rendering | **ACCEPTED 2026-08-08.** Small, docs/CI only. Diagrams in the two new reference docs emit `language-mermaid` today. Requires one **exact** `procedure.rs` allowlist entry — a security-control change, hence an increment rather than a config edit | nothing | `handoffs/DC-77-docs-mermaid-rendering/implementation-handoff-v1.md` |
| 18 | **DC-78** — history exchange | **ACCEPTED 2026-08-09.** Status-claim criterion 1, the only one with no increment behind it. Separates **exchange** (trust — the hard part) from **transport** (a dependency decision, deferred). **§4 investigation only**; design gated on it | nothing — but **DC-81 has priority** (ruled 2026-08-09; DC-76 is complete, so that earlier note is superseded). §3.2 may show DC-53 is a prerequisite, which would change this increment's position; that question is cheap and may be answered out of band at any time. **Hand-over trigger, recorded 2026-08-09 at the owner's request:** DC-78 goes to the developer at the earliest of — (a) DC-81's §1 report submitted and with the architect for review, (b) DC-81 blocked on a ruling or on macOS CI runner availability, (c) DC-81 complete. §4 here is read-only and cannot collide with DC-81, so the trigger is capacity, not conflict. **The architect raises this unprompted** | `handoffs/DC-78-history-exchange/implementation-handoff-v1.md` |
| 19 | **DC-79** — sha2 + getrandom upgrade | **ACCEPTED 2026-08-09.** Not urgent (audit clean). `sha2` derives every `ObjectId` — proof is DC-41's vectors unchanged and **no edited hash literal** | nothing; cannot collide with DC-76/DC-78 | `handoffs/DC-79-sha2-getrandom-upgrade/implementation-handoff-v1.md` |
| 20 | **DC-80** — ed25519-dalek 2→3 | **ACCEPTED 2026-08-09**, after DC-79. Verifies every signature in the system. **Both failure directions must be tested** — the silent one is 3.x accepting what 2.x rejected | DC-79 | `handoffs/DC-80-ed25519-dalek-upgrade/implementation-handoff-v1.md` |
| 21 | **DC-81** — macOS mutation | **ACCEPTED 2026-08-09**, 0.20.0 item 2. The increment the owner's cross-platform priority points at. **First prerequisite is not durability: APFS is case-insensitive by default, which meets NFR-SEC-03 head-on.** Neither developer nor architect can verify locally — a macOS mutation CI job must exist and be green before any gate is relaxed | DC-76 (complete) **Carries the gate-reduction target** (RFC §6): call sites unconditional, implementation modules gated, `not(any(...))` fallback preserved for DC-71, **single-digit `fsutil/` gate count as a criterion for the Windows increment** | `handoffs/DC-81-macos-mutation/implementation-handoff-v1.md` + `-addendum.md` |
| 22 | **DC-82** — mutation dispatch collapse | **ACCEPTED 2026-08-09**, before Windows. Behaviour-preserving refactor, kept separate from the Windows backend so each has its own proof — the DC-76/DC-81 split, which is how DC-76's B1 was found | DC-81 closing | `handoffs/DC-82-mutation-dispatch-collapse/implementation-handoff-v1.md` |
