...
| 10 | **DC-70** — prebuilt binary distribution | **In progress, partially escalated 2026-08-03.** §2/§3's four prerequisites answered (`prerequisite-questions-v1.md`): targets verified by trial build — Linux only, `x86_64`/`aarch64`; Windows found not to compile at all off Linux (new, unowned finding, not fixed here). Evidence-schema extension (criterion 3) found to sit inside DC-45's frozen-until-0.19.0-cutover oracle — escalated, no mechanism proposed, no `release/oracle`/`policy_check`/`schemas` edits made. Everything else proceeding | nothing on the non-schema work; a ruling awaited on criterion 3's disposition | `handoffs/DC-70-prebuilt-binary-distribution/implementation-handoff-v1.md` |
