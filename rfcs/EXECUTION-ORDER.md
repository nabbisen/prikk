...
| 7 | **DC-57** — active-patch thresholds (NFR-PERF-02) | **Implemented.** "Active patches" defined once (active WAL record count) and enforced through one shared comparison every authoring path calls; warn at 800 extends DC-66's `status` output; hard block at 1000 fires before any WAL append or object write, proven to leave no partial state; both configurable via `PRIKK_ACTIVE_PATCH_WARN`/`PRIKK_ACTIVE_PATCH_LIMIT`, malformed values rejected rather than defaulted; `seal` confirmed available at and above the bound; boundary tested at the RFC's literal 799/800/999/1000/1001 plus a scaled-limit integration proof | none | — |
| 8 | **DC-67** — ordinary-use conformance suite | **Implemented. Prediction held — two findings, neither fixed here** (per criterion 4): (1) `checkout --patch-materialize` does not replay `ReplaceBinary` or `ChangePerm` — pre-existing, but shown here to block criterion-2 rebuild verification for two named ordinary sequences, not just adversarial ones; (2) no "checkout branch into the worktree" step exists for active multi-branch editing, a real capability gap. Nine sequences at N=3 (sequence 1 kept from DC-65); shared CLI harness consolidated (`tests/support/`); suite runs in the ordinary gate (0.33s) | none | — |
