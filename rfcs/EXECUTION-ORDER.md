...
| 9 | **DC-69** — lifecycle-state retention | **In progress, escalated at `c17268f`.** §3.1 judgment answered (the mint-collision guard is not load-bearing for any currently-reachable threat) and §3.4 measured (Axis D: cumulative history has a real, roughly-linear, tree-size-independent cost — 2.66 ms at depth 10 to 17.91 ms at depth 200, fixed 20-file tree throughout). **§3.2's discharge is contradicted**: `create_node`'s restoration-equivalence check (in `prikk-replay`, reached from ordinary commit-path replay via `rollback-draft`'s node-id-reusing inverse patches, not only the merge path as discharged) consumes tombstone content on the commit path. §3.3 and a final verdict are withheld pending an architect ruling — see `.git-exclude/review-request/prikk-dc-69-design-question-v1.md` | architect ruling on the §3.2 contradiction | `handoffs/DC-69-lifecycle-state-retention/implementation-handoff-v1.md` |
