...
| 14 | **DC-74** — merge execution | **ACCEPTED 2026-08-08**, roadmap item B, largest increment on the roadmap. Merge **adopts** patches verbatim — signatures survive because operations are context-free (stable `NodeId` + content anchors). Single-parent blocks this increment; multi-parent lineage deferred, not rejected | nothing — **but §4's prerequisite questions must be answered and reported before any design**, and if sealing cannot adopt a foreign-authored patch, B′ is unavailable and it returns to the owner | `handoffs/DC-74-merge-execution/implementation-handoff-v1.md` |
