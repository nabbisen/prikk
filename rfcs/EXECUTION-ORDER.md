...
| 18 | **DC-78** — history exchange | **DESIGN ACCEPTED by the owner 2026-08-09** (RFC §D). Adopted-key **set**, with object-trust separated from ref-authority: an adopted key makes a peer's blocks valid objects but **cannot advance any ref**. Provenance is already intrinsic (the sealer key id inside the signature) — **no new mechanism**. **Key finding: single-key trust makes adoption destructive**, since `verify` checks every block against the policy | nothing — **§D7 answered before code** | `handoffs/DC-78-history-exchange/implementation-handoff-v2.md` |
