...
| 11 | **DC-71** — non-Linux build conformance | **Implemented 2026-08-04, awaiting architect review.** §3's three questions answered (`prerequisite-questions-v1.md`); `fsutil/anchored/{immutable,regular,read}.rs`/`anchored.rs`/`directory.rs`/`fsutil.rs` cfg-gating repaired, verified clean via `cargo clippy --target x86_64-pc-windows-gnu`/`x86_64-apple-darwin -- -D warnings`; the 25-command read-only/mutation boundary traced and published (`docs/src/reference/platform-support.md`); new CI jobs build both targets on every change and run the read-only set against a real fixture repository. One new finding (`worktree-status` unreachable via ordinary use) recorded, not fixed | nothing outstanding on the developer | `handoffs/DC-71-non-linux-build-conformance/implementation-handoff-v1.md` |
