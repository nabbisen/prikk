...
| 6 | **DC-66** — multi-commit queuing | **Implemented.** §4's four questions answered before design (`prerequisite-questions-v1.md`); the baseline-for-the-next-queued-patch rule stated and implemented as a chain fold (`queuing-baseline-design-v1.md`); node identity across a queue proven safe; `seal` batching and DC-64/DC-65 both tested at N > 1 for the first time (load-bearing); crash recovery covers a torn queue and a crash during seal with no silent loss; `verify`/`status` report queue health. Unblocks DC-57 (capability only — threshold values still unset) | none | — |
