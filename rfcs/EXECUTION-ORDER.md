...
| 23 | **DC-80** — ed25519-dalek 2→3 | **ACCEPTED 2026-08-09**, after DC-79. Verifies every signature in the system. **Both failure directions must be tested** — the silent one is 3.x accepting what 2.x rejected | DC-79 | `handoffs/DC-80-ed25519-dalek-upgrade/implementation-handoff-v1.md` |
| 24 | **DC-85** — merge from a received ref | **ACCEPTED 2026-08-09.** DC-78 Stage 3 showed `execute_merge:65` rejects `remotes/`, so exchange is complete for an auditor and incomplete for a collaborator — **the architect's §D4 claim that this used existing machinery was false**. Needs a new `MergeEvidenceTarget` and a ruling on confluence against a source with no local publication chain | nothing — **§3's four questions precede design** | `handoffs/DC-85-merge-from-received-ref/implementation-handoff-v1.md` |
