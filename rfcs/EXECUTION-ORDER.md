...
| 9 | **DC-69** — lifecycle-state retention | **Complete.** Ruling accepted: §3.2's discharge withdrawn, narrowed to a checkable invariant ("a horizon may not sever a `DeleteNode` from a later restoring `CreateFile` of the same node id"). §3.3 answered under that invariant — a horizon mechanism is conceivable but depends on two decisions outside this increment's scope (bounding `rollback-draft`'s reach; redefining what full replay trusts below a horizon). **Verdict: route (c)**, established and measured, not assumed — recorded in `MILESTONES.md`. DC-64's binding condition 1 unchanged | nothing | — |
