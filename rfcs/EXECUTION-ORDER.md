...
| 18 | **DC-78** — history exchange | **STAGE 1 COMPLETE, merged 2026-08-09** at `03a21b4`. Trust store is an adopted-key **set** with TOFU enforced on conflicting re-add; **adopting a second key no longer invalidates the receiver's own history** (§D1's bug, now unreachable). Both negative controls discriminate precisely — architect ran independent versions. **Five false single-key doc claims found and fixed across two sweeps** (developer found 3, architect 1, developer's tree-wide sweep 1 more). **Remaining stages: D3, then D4/D6 + ruling 4 together** | nothing | `handoffs/DC-78-history-exchange/implementation-handoff-v2.md` + addenda 1-3 |
