...
| 10 | **DC-70** — prebuilt binary distribution | **Closes partial, reviewed 2026-08-03 (DC-56 precedent).** Linux-only binaries (`x86_64`/`aarch64`), `cargo binstall`, and download-surface statements implemented and accepted. **Criterion 3 carried** — the evidence schema sits inside DC-45's frozen-until-0.19.0-cutover oracle; extending it needs its own review, ruled out of this increment's scope. One review finding (B1: three `inert_head` entries were unsafe with any arguments) repaired and re-verified | nothing outstanding on the delivered scope; criterion 3 owned by a future increment coinciding with or after the DC-45 cutover | `handoffs/DC-70-prebuilt-binary-distribution/implementation-handoff-v1.md` |
