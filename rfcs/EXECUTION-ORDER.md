...
| 14 | **DC-74** — merge execution | **ACCEPTED 2026-08-08**, roadmap item B, largest increment on the roadmap. Merge **adopts** patches verbatim — signatures survive because operations are context-free (stable `NodeId` + content anchors). Single-parent blocks this increment; multi-parent lineage deferred, not rejected | nothing — **but §4's prerequisite questions must be answered and reported before any design**, and if sealing cannot adopt a foreign-authored patch, B′ is unavailable and it returns to the owner **Prerequisites answered and accepted 2026-08-08** — B′ confirmed available. **Release-conditioned**: buildable now, not releasable until sealed history structurally records a merge | `handoffs/DC-74-merge-execution/implementation-handoff-v1.md` + `-addendum.md` |
| 15 | **DC-75** — merge block lineage / structural merge record | **ACCEPTED 2026-08-08.** Discharges DC-74's release condition. Gate is `block_state.rs:13-26` rejecting `BlockKind::Merge` outright — greenfield write-side design. Carries one open design question: mainline-authoritative vs both-parents-verified | **§4 investigation: nothing, may start now** (read-only). **§5 implementation: DC-74 merging first** — both touch the seal path | `handoffs/DC-75-merge-block-lineage/implementation-handoff-v1.md` |
