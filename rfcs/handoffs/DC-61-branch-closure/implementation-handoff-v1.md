...
**Size:** medium. One payload field, schema-aware decoding through 10 call sites, one CLI subcommand, one
list filter.
**Touches:** `prikk-object` (`RefStatePayload`, encode + decode), 10 non-test decode call sites,
`prikk-cli` (`branch close`, `branch list --all`). **Not** `verify`, **not** `publish`, **not**
`recoverable_missing_ref`, **not** `doctor` — see §"The falsification test".
...
`doctor` need no change. Obligation 2 confirmed that:
...
**If you find yourself needing to modify any of those four, stop and report it.** Acceptance criterion 4
requires the diff to leave them untouched. A diff that touches them does not complicate the design — it
**refutes** it, and the choice between closure and tombstones has to be reopened. That is a stop-work
...
## Step 2 — schema-aware decoding, across 10 call sites
...
crates/prikk-store/src/refs/publication.rs:128
...
22 including tests. Each has the envelope in hand, so `schema_version` is available at every site — this is
...
- **Modifying any of the four functions in the falsification test.** Stop and report instead.
...
schema-aware decoding through all 10 call sites; `branch close` publishing a maintainer-signed schema-2 ref
state with the pointer intact; the three fail-closed conditions; `branch list` filtering with `--all`;
reopening working as an ordinary CAS update; output stating nothing was reclaimed; format-transition
behaviour evidenced; **`verify`, `publish`, `recoverable_missing_ref`, and `doctor` unmodified.**

## Submit with

The diff; **explicit confirmation that the four falsification-test functions are unmodified**; evidence that
an ordinary (non-closed) ref state's payload bytes and ObjectId are unchanged — this is the identity claim
and it should be asserted by test, not by inspection; a commit to an unrelated ref succeeding after a closure
(criterion 2, the DC-60 regression); corruption detection still reported and blocking for pointer-absent-log-present
at every record count (criterion 3, tested by simulating pointer loss as
`seal_rejects_missing_pointer_with_ref_log_history` does); the format-transition evidence; test counts per
touched crate before and after; and the full gate set from `rfcs/EXECUTION-ORDER.md` §6 rule 9 run on a
**clean checkout of the commit**, stated as such.
...
been. If something here contradicts a shipped RFC, an accepted requirement, or the four functions the design
depends on leaving alone, stop and report it.
