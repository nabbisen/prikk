...
| 101 | [First-Appearance Durability](./proposed/101-first-appearance-durability.md) | **Proposed 2026-08-12** on the owner's parity direction. Windows cannot make a newly-created name durable, so DC-38's invariant breaks at ref creation on every per-ref file shape (DC-91 §3). Routes durability-bearing transitions through already-existing names. **First RFC under RFC 100's `NNN-slug` naming.** 0.20.0 moves if accepted |
