...
| DC-71 | [Non-Linux Build Conformance](./proposed/DC-71-NON-LINUX-BUILD-CONFORMANCE.md) | Product **M1**. `prikk-store` **does not compile at all** off Linux — inconsistent cfg-gating in `fsutil/anchored/*`, found by DC-70's trial builds. Contradicts DC-37's stated design and closes the long-standing public-portability-claim mismatch. **Blocking owner question: is portable read-only a requirement or an aspiration?** Awaits acceptance |
