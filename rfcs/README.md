...
| 121 | [The CLI's boundary contract](./proposed/121-cli-boundary-contract.md) | Unscheduled. External audit 2026-08-31. EPIPE panic (reproduced), exit-code contract, per-command help, arg-parser hygiene, four undocumented flags. **Owner ruling: the exit-code contract's shape** |
| 122 | [`worktree-status` baseline repair](./proposed/122-worktree-status-baseline-repair.md) | Unscheduled. External audit 2026-08-31, **High**. The command fails on every repository this CLI produces; reproduced. Known and recorded at `ROADMAP.md:177`, but `README.md:256` still advertises it without a caveat |
| 123 | [Commit message and authorship metadata](./proposed/123-commit-message-and-authorship-metadata.md) | Unscheduled. External audit 2026-08-31, **High**. `-m` is required, validated, and discarded. **Owner ruling required and the RFC deliberately does not pre-empt it**: is a message evidence (inside identity, as `TagPayload.message` already is) or annotation? A commit *date* is structurally excluded — `created_at` is pinned to 0 and CI proves cross-platform byte-identity from it |
| 124 | [Worktree ignore mechanism](./proposed/124-worktree-ignore-mechanism.md) | Unscheduled. External audit 2026-08-31. No ignore at any layer, and the no-GC model makes a mistaken first commit permanent. The README disclosure can ship before the design |
...
| 126 | [Verification infrastructure coverage](./proposed/126-verification-infrastructure-coverage.md) | Unscheduled. External audit 2026-08-31. Zero property tests in the patch algebra despite a ready oracle; `cargo audit` absent from CI; docs ungated; benchmarks never run. **Owner ruling: criterion as a dev-dependency** |
...
| 128 | [Outward-facing project surface](./proposed/128-outward-facing-project-surface.md) | Unscheduled. External audit 2026-08-31. No `SECURITY.md`, no `CONTRIBUTING.md`, 7 of 8 library crates publish with no categories/keywords/homepage, no Git→prikk mapping page. **Owner ruling: the disclosure channel** |
...
128 cannot start without an owner ruling.**
