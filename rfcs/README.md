...
| DC-56 | [Commit Scan and Memory Compliance](./accepted/DC-56-COMMIT-FULL-TREE-SCAN-COMPLIANCE.md) | Closes **missed product M1** gate NFR-PERF-01 plus an untracked commit-memory defect. Accepted 2026-07-30; owner ruled the requirement bounds steady-state cost, so a changed-path index is permitted with a cache-validity spec and divergence detection required. Implementation review needs DC-62 |
| DC-62 | [Commit Benchmark Memory Axis](./accepted/DC-62-COMMIT-BENCHMARK-MEMORY-AXIS.md) | Precondition for DC-56's implementation review. Accepted 2026-07-30; measures peak commit memory with no new dependency, by sampling `/proc/<pid>/status` `VmHWM` |
| DC-63 | [Tag Surface](./accepted/DC-63-TAG-SURFACE.md) | §6.6. Held briefly 2026-07-30 — `publish` rejected every `tags/` name and `verify` required every ref target to be a `Block`, with `RefKind::Tag` having zero production call sites. Both fixes now specified and fix-reviewed; **cleared via handoff v2** |
