...
| DC-74 | [Merge Execution](./proposed/DC-74-MERGE-EXECUTION.md) | Product **M3**, roadmap item B. `merge-evidence`/`merge-plan` exist; nothing applies a merge. **Carries an owner fork**: a two-parent block would invalidate the single-parent replay model, DC-64's cache, and rollback-preview at once — verified, not assumed. Awaits acceptance |
