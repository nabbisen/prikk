...
| DC-67 | [Ordinary-Use Conformance Suite](./proposed/DC-67-ORDINARY-USE-CONFORMANCE.md) | Corrective assurance. Opened 2026-08-02 after **three consecutive increments** whose decisive defect was found by running sequences, not by inspection or any existing gate. Adds the orthogonal axis to DC-41's adversarial coverage; carries a falsifiable prediction that it will find at least one more defect; awaits owner acceptance |
...
| DC-57 | [Active-Patch Thresholds](./accepted/DC-57-ACTIVE-PATCH-THRESHOLDS.md) | **HOLD LIFTED 2026-08-02, cleared to start** (handoff v2; v1 withdrawn). Its original premise (the active WAL capped at one record, making 800/1000 unreachable) no longer holds — **DC-66 is implemented**, so multi-commit queuing is a real, tested capability. NFR-PERF-02 needs no amendment. DC-57 is now blocked on choosing and implementing the actual threshold values, not on capability or a decision |
