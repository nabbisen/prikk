...
| DC-70 | [Prebuilt Binary Distribution](./accepted/DC-70-PREBUILT-BINARY-DISTRIBUTION.md) | Adoption surface. **Accepted 2026-08-03.** `cargo install prikk` works as of 0.18.1 but needs a Rust toolchain; prebuilt binaries on GitHub Releases remove that and unlock `cargo binstall`. **Blocking prerequisite:** release evidence models a singular archive, and per-target binaries are N artifacts — a schema question before a workflow question. Must not create the appearance of signed releases |
