...
| DC-66 | [Multi-Commit Queuing](./accepted/DC-66-MULTI-COMMIT-QUEUING.md) | Product **M3**. **Accepted 2026-08-02** on the owner's decision that queuing is a scheduled capability. Unblocks **DC-57**; makes NFR-PERF-02/03 meetable without amendment. **Scope warning in its §2**: the block format and `seal` already support N, so the cap is three guards — the increment is crash recovery, `verify`, and ordering, not the append path |
