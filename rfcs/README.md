...
| DC-63 | [Tag Surface](./accepted/DC-63-TAG-SURFACE.md) | §6.6. **HELD 2026-07-30, handoff withdrawn** — `publish` rejects every `tags/` name and `verify` requires every ref target to be a `Block`, so no tag can be published or verified. `RefKind::Tag` has zero production call sites. Both fixes specified in the RFC; no shippable slice |
