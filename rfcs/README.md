...
| DC-66 | [Multi-Commit Queuing](./accepted/DC-66-MULTI-COMMIT-QUEUING.md) | Product **M3**. **Implemented.** The active session holds N unsealed patches; `commit` no longer refuses on a non-empty active WAL; `seal` batches the queue into one block. Baseline-for-the-next-queued-patch chain rule stated and implemented; node identity across a queue proven safe; DC-64's incremental cache and DC-65's text materialization both tested at N > 1 for the first time; crash recovery covers a torn queue and a crash during seal with no silent loss; `verify`/`status` report queue health. Unblocks **DC-57** (capability only — threshold values still unset) |
