...
| DC-65 | [Text-Edit Baseline Content Availability](./accepted/DC-65-TEXT-EDIT-BASELINE-CONTENT.md) | Product **M1**, **severe correctness defect** — editing one text file in two separate sealed commits fails. Long-standing; reproduced by the architect on `6064da6`. **Implemented.** Invariant established: a `TextFile` node's `blob_id` is a content identity, not necessarily a stored object; `plan_edit_text` and `patch_algebra::baseline_text` now materialize via replay on a miss. A fifth site (DC-64's incremental step) was found and fixed once the first three sites unblocked it. Verified at N=4/5 sealed edits, `ReplaceBinary` confirmed unaffected, coverage finding recorded (`rfcs/handoffs/DC-65-text-edit-baseline-content/implementation-summary-v1.md`) |
