...
| DC-70 | [Prebuilt Binary Distribution](./accepted/DC-70-PREBUILT-BINARY-DISTRIBUTION.md) | Adoption surface. **In progress, partially escalated 2026-08-03.** Targets verified by trial build: Linux (`x86_64`/`aarch64`) only — Windows found not to compile off Linux at all, a new unowned finding, not fixed here. The evidence-schema extension (release evidence models a singular archive; per-target binaries are N artifacts) turned out to sit inside DC-45's frozen-until-0.19.0-cutover oracle corpus — escalated rather than edited; everything else (workflow, `cargo binstall`, download-surface release-authority and mutation-limit statements) proceeding regardless. Must not create the appearance of signed releases |
