...
| DC-69 | [Lifecycle-State Retention](./accepted/DC-69-LIFECYCLE-STATE-RETENTION.md) | Design increment, **in progress, escalated at `c17268f`**. §3.1 answered (the mint-collision guard defends no currently-reachable threat) and §3.4 measured (a new small-tree/long-history benchmark axis: cost grows ~linearly with cumulative history alone, independent of tree size). §3.2's architect discharge is contradicted — `create_node`'s restoration-equivalence check consumes tombstone content on the **commit** path too, via `rollback-draft`'s node-id-reusing inverse patches, not only the merge path as discharged. §3.3 and a final "does prikk forget" verdict withheld pending an architect ruling |
