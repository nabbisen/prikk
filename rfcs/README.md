...
| 121 | [The CLI's boundary contract](./proposed/121-cli-boundary-contract.md) | Unscheduled. External audit 2026-08-31. EPIPE panic (reproduced), exit-code contract, per-command help, arg-parser hygiene, four undocumented flags. **RULED by the architect 2026-09-01: `0` ok / `1` operational failure / `2` usage error.** Option 2's separate integrity code was refused — `verify --format json` already carries that verdict, and duplicating it into an integer is what RFC 118 exists to prevent. The ruling surfaced a sharper defect than the audit's: `unlock`'s confirmation resolves to "no" on EOF, so a non-interactive `prikk unlock && proceed` exits 0 with the lock still held |
