...
| 115 | [Sync: what it can mean for prikk](./proposed/115-sync-investigation.md) | **Investigation, not design.** Started 2026-08-19 on the owner's direction, for badge criterion 1. **Corrects the board**: "nothing built" is false — `bundle export`/`import`, the `remotes/` received namespace, merge-from-received-ref (DC-85) and authorship verification of imported history (DC-53 Stage 2) all exist and are tested. What is missing is **transport** and **repository-complete transfer**. Poses the question that decides scope: is sync a protocol prikk owns, or a complete verifiable artifact plus any transport the user has? **No design exists** |
...
**Scheduled:** **115** is the current work — badge criterion 1's prerequisite investigation.
