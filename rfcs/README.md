...
| DC-71 | [Non-Linux Build Conformance](./accepted/DC-71-NON-LINUX-BUILD-CONFORMANCE.md) | Product **M1**. **Accepted 2026-08-04 with an owner ruling: portable read-only is a requirement.** `prikk-store` does not compile at all off Linux — inconsistent cfg-gating in `fsutil/anchored/*`. Closes the long-standing public-portability-claim mismatch. Mutation stays Linux-only per DC-37 |
