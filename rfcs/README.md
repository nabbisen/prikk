...
| DC-69 | [Lifecycle-State Retention](./accepted/DC-69-LIFECYCLE-STATE-RETENTION.md) | Design increment, **accepted 2026-08-03**. `seen_ids` and `latest_tombstone_by_id` grow with **cumulative history, forever**, and DC-64 must persist the first complete on every commit — an architectural ceiling, not a performance ticket. Asks whether prikk forgets at all; "inherent" is a permitted outcome |
