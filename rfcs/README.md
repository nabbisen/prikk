...
| DC-70 | [Prebuilt Binary Distribution](./proposed/DC-70-PREBUILT-BINARY-DISTRIBUTION.md) | Adoption surface. `cargo install prikk` works as of 0.18.1 but needs a Rust toolchain; prebuilt binaries on GitHub Releases remove that and unlock `cargo binstall`. Blocking prerequisite: release evidence models a **singular** archive, and per-target binaries are N artifacts. Must not create the appearance of signed releases; awaits owner acceptance |
