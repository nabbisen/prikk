...
| 104 | [Windows Mutation](./accepted/104-windows-mutation.md) | **Accepted 2026-08-16**, the other half of RFC 102's arc. RFC 102 removed the obstacle — `durable_directory_entry`, the primitive Windows cannot provide, now has only worktree callers and none under `.prikk/` — but did not build the capability. **Not a port, unlike DC-81**: `MutationRoot`'s anchored directory handle has no `openat` on Windows, `rustix` is POSIX so a policy-gated new dependency is near-certain, and two contract methods need argued semantics. Supersedes DC-37. §4.1 (`MutationRoot`) is blocking. **1.0-scale**, per RFC 102 §9 |
