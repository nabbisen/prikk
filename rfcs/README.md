...
| DC-74 | [Merge Execution](./proposed/DC-74-MERGE-EXECUTION.md) | Product **M3**, roadmap item B. `merge-evidence`/`merge-plan` exist; nothing applies a merge. Patches are **context-free** (stable `NodeId` + content anchors), so merged patches transport bit-identically and author signatures survive — a merge must **adopt**, never synthesize. `parent_block_ids` is already `Vec`, so multi-parent is a replay question, not a format one. Awaits acceptance |
