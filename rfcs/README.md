...
| DC-68 | [Preview Release Authority Class](./accepted/DC-68-PREVIEW-RELEASE-AUTHORITY-CLASS.md) | **Release-blocking. Accepted 2026-08-02** by the owner's Option C approval. DC-35's two-person signer rule and its protected-branch authority root both fail for a single-person project — `main` verified unprotected. Scopes both to an official class; defines a `preview` class that must declare in its evidence `governance` record what it did not receive. **No release at any version until this lands** |
