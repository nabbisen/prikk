...
- **DC-76's negative controls are only partly demonstrated on Windows, for the eight guarantees
  ...
  above for the per-guarantee table and reasons. G1, G2, G4, and G9 are demonstrated; G3, G6, G7,
  and G8 are not. Not all of the missing ones turn on the same cause: G3 and G8 are blocked on the
  Windows failpoint injection mechanism being unbuilt (Linux/macOS-only today); G6 and G7 have no
  Windows analogue to demonstrate at all, for reasons unrelated to failpoints.
