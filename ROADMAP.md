...
- [`136-block-aggregation-payoff.md`](rfcs/proposed/136-block-aggregation-payoff.md) — RFC 136, The block aggregation payoff: what sealing a block should make cheap (opened 2026-09-04 on the owner's instruction, answering their own question about the original block concept; **nothing ruled** — the block already bounds merge work to the sealed window, but `snapshot_blob_ref` is written `None` by all three block-creating paths and `SnapshotManifest::encode` is called only from tests, so the materialization half was never built; §7 carries the owner's question — whether the acceleration should travel between repositories or stay local)
<!-- open-work-index:end -->
