...
- [`138-trust-policy-read-surface.md`](rfcs/proposed/138-trust-policy-read-surface.md) — RFC 138, Asking what a repository trusts (requested by the stikk project 2026-09-06; **owner approved it as worth doing the same day**). No way exists to ask which MAINTAINER keys a repository trusts — and **`verify`'s `sealed-block <id>: <key_id>` is historical signer attribution, not current policy, and does not exist before the first seal**, which is exactly when a seal ceremony asks. `load_maintainer_trust_policy` already returns the answer; only the CLI surface is missing. **Found while writing it: `policy: required=1` is a hard-coded literal in two places, printed as if read** — there is no threshold field, trust is any-of-N. **ACCEPTED 2026-09-06 and §4 ruled by the architect the same day**: **both** `list` and `check`, with **`--format json` riding along** — not a nicety, but what keeps `check` inside RFC 121's exit-code contract, since "key X is not trusted" is a successful answer and exiting `1` for it would file it as an operational failure, the exact conflation stikk reported in their first letter. Nothing awaits the owner; handoff next)
