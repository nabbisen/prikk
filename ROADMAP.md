...
- **DC-29 verify and doctor integrity/recovery reference (implementation review).** Added a
  current-state mdBook reference for what `prikk verify` checks and does not prove, how `prikk doctor` interprets
...
- No active release candidate is selected after the 0.17.2 release.
...
| TASK-07 verify & doctor reference | 1 | Architect + maintainer | DC-29 implementation review | Review the implemented verify/doctor integrity-recovery reference. | Reviewed integrity/recovery docs define what `verify` and `doctor` do and do not prove. | `docs/src/reference/integrity-recovery.md` |
