...
- [`134-text-span-identity-under-composition.md`](rfcs/proposed/134-text-span-identity-under-composition.md) — RFC 134, A text span's identity is not stable under composition (found 2026-09-03 by RFC 126 §2's property tests; **ACCEPTED IN FULL 2026-09-04 and all ruled work delivered** — shape 1's containment at `59fa298`, §8's v2 identity at `906d015`/`6076d52`, `2c8289a` repairing the two non-Linux jobs; **corrected 2026-09-04: NOT user-reachable** — the first ruling said reachable through `merge` and was wrong, see §3; **corrected: not a format break** — an additive schema version under RFC 114, see §7.2. **What remains is a watch, not work**: v1 operations depend on the sequencing invariant permanently, and RFC 113's import is the named direction that would violate it)
- [`135-first-run-entrance-and-configuration.md`](rfcs/proposed/135-first-run-entrance-and-configuration.md) — RFC 135, The entrance: what a new user meets before anything works (raised by the owner 2026-09-04; **deliberately unhurried, nothing ruled**; 23 commands and none generates or derives a key, and the dependency question is open and decidable by DC-50's ROI method, not closed by the current allowlist)
...
- [`137-project-entrance-landing-page.md`](rfcs/proposed/137-project-entrance-landing-page.md) — RFC 137, The project's entrance: a landing page, and how it stays true (owner theme 2026-09-04; the web entrance is `docs/src/index.md`, a 19-line table of contents doing a front door's job). **Hosting and content decisions all settled by the owner the same day** and recorded in §3 rather than re-opened: `prikk.org`, docs at `/docs/` as a subdirectory, one repository, source at `docs/landing/`, mdBook root unchanged. **This RFC's own content is §4 — how an HTML page stays true when every currency gate here is a Markdown parser**: two drafts written the same week carried three false claims and a stale URL, and declaring HTML in `DECLARED_DOCUMENTS` today would pass vacuously. Ruled: extend `code_regions` for `<code>`/`<pre>`, **before the page lands**. §5 divides the three entrance surfaces; §8 couples it to RFC 135 at the install/first-run seam. Five increments, only the last waiting on the domain)
<!-- open-work-index:end -->
