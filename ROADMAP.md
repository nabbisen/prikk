...
- [`135-first-run-entrance-and-configuration.md`](rfcs/proposed/135-first-run-entrance-and-configuration.md) — RFC 135, The entrance: what a new user meets before anything works (raised by the owner 2026-09-04; **§9 DESIGNED 2026-09-06**). **`prikk config` deferred with a named trigger — which removes §4's dependency/config-format question from the critical path entirely, by making it unnecessary rather than answering it.** `prikk key` and `prikk setup` ruled in; a credential-helper boundary deliberately refused. **§6 answered: prikk stores no secret**, but may write one to a path the user names (`ssh-keygen` model), never inventing a location or reading it back. Seeds are read from the environment and **never from argv** — `/proc/<pid>/cmdline` is world-readable. **§9.8's two questions withdrawn and ruled 2026-09-06** on the owner's challenge — `--out` is in because printing a seed to a terminal is the *less* secure option, and `setup` is a single command composed over a first-class sequence that must show the trust decision it makes. **Design complete; handoff next**)
