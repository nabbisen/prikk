...
- [`140-queued-patch-enumeration.md`](rfcs/proposed/140-queued-patch-enumeration.md) — RFC 140, Which patches, not how many (opened 2026-09-06 after the owner authorized part 3 of the stikk letter 003 round. **Opened because the architect's own recommendation understated the work**: `EditText` is node-addressed, not path-addressed, so "patch ids and their target paths" is **not** derivable from the queue alone — resolving it needs the folded baseline, a derivation `status` does not currently do. Recommends option (b), resolution against the folded baseline, on the bound that `commit` already runs the same derivation. **Needs owner acceptance**; parts 1 and 2 of that round are already handed off)
...
**137** (increments 1-4 shipped in 0.31.1, increment 5 blocked on `prikk.org` DNS), and **139** (the
measurement corpus, owner-accepted 2026-09-06 the day it was opened, unimplemented). **130 and 138
