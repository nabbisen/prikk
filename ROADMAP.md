...
- [`136-block-aggregation-payoff.md`](rfcs/proposed/136-block-aggregation-payoff.md) — RFC 136, The block aggregation payoff: what sealing a block should make cheap (opened 2026-09-04 on the owner's instruction, **ACCEPTED the same day**; acceptance covers the problem record and clears §9's measurements only — **§7 remains the one open question**, whether the acceleration should travel between repositories or stay local. The block already bounds merge work to the sealed window, but `snapshot_blob_ref` is written `None` by all three block-creating paths and `SnapshotManifest::encode` is called only from tests, so the materialization half was never built. **§9.1 measured 2026-09-04 eliminates Option B** — composition collapses only 1.16-1.25x at realistic block sizes and that figure is a ceiling, so the architect recommends against the option closest to the original wording; Options A and C are untouched)
<!-- open-work-index:end -->
