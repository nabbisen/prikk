...
- **DC-33 - concurrency and locking reference (accepted; ready for implementation).** Documentation-only
  0.17.7 increment for TASK-12. The implementation should add a current-state mdBook reference for
  active-session locking, ref-specific publication locks, compare-and-swap behavior, narrow ref repair
  locking, and manual stale-lock limits. It must not change code, schema, CLI behavior, lock behavior,
  repository behavior, verification, doctor, trust, release semantics, or repository-format stability
  claims.
...
| TASK-12 concurrency & locking model | 2 | Architect + maintainer | DC-33 accepted | Implement accepted DC-33; coordinate with TASK-06, especially stale `active.lock` and CAS behavior. | Reviewed locking/concurrency docs are committed and describe manual stale-lock limits. | `docs/src/reference/concurrency-locking.md` |
