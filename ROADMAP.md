...
- [`123-commit-message-and-authorship-metadata.md`](rfcs/proposed/123-commit-message-and-authorship-metadata.md) — RFC 123, The commit message is validated and then discarded (external audit 2026-08-31, High; **ruled 2026-09-01** — the message is evidence; the `note:` interim shipped). **§8 DESIGNED 2026-09-05**: `message: Option<String>` at `PatchPayload` tag 6, `Patch` schema 4; **no length bound, deliberately** — the same object already carries two unbounded user text fields; **optional not required**, because RFC 113's Git import must be able to represent a commit that genuinely had none; **`verify` gains nothing**, stated as a negative. **The expensive part is not the field**: the decoder refuses unknown tags, so schema 4 is **a second one-way break on every commit**, same shape as 0.31.0's — whether it ships alone or batched is the owner's release decision. No handoff issued)
