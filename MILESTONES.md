...
**Assessed 2026-08-24, badge unchanged — owner authorized a change, the architect recommended against
it, and the owner accepted.** Five of six criteria are now MET (1, 2, 3, 5 all met since `0.22.1` and
shipped in `0.23.0`; 6 met at `0.21.0`). **Criterion 4 remains open by the owner's own ruling** (*"not
wrong and just too early to be applied"*). But the decisive evidence was the paired sentence, not the
count: **`0.23.0` shipped a breaking change to a shipped payload** — a repository written by `0.22.1`
holding a tag cannot be repaired under `0.23.0`, and `verify` fails on it. **A user who followed the
previous release's instructions holds history the current release refuses.** That is *"future releases
...
|---|---|---|---|
| 1 | **Sync exists** — two machines can exchange sealed history, and both verify it afterward | A *distributed* VCS that cannot distribute. **Corrected 2026-08-19 by RFC 115's investigation: the second half of this row was wrong.** Exchange between repositories does exist — `bundle export`/`import` move a genesis-complete closure, imported history lands in the `remotes/` received namespace without advancing any local ref (DC-78, by design), `merge` accepts a received ref as one side (DC-85), and since DC-53 Stage 2 a receiver cryptographically verifies imported authorship. **All of it is tested end to end** | **OPEN, and exactly one gap remains: transport.** **RFC 115 is implementation-complete and merged 2026-08-20** — patch-level exchange as the owner ruled it (§2.2), across five increments, each reviewed and merged at the exact reviewed commit: the patch-set digest (`bb5424c`), the signed recognition claim (`a56535a`), the `PEXCH001` exchange artifact and accept path (`0128c91`), the D6 amendment making the claim carry block order verbatim (`1e72235`), and seal-from-accepted (`07d8a47`). **A receiver can now accept foreign patches, verify their AUTHOR signatures against transported key material, store them, see what it holds unsealed, and seal it under its own maintainer key in the order a signed claim supplies.** Trust never expands on receipt: no artifact can cause a maintainer key to be adopted. Divergence — an accepted patch not applying to the receiver's tip — is classified and reported **as divergence, not corruption**. `verify` also gained a received-namespace stage, closing the gap where a dangling received ref was invisible on both sides. Ten security refusals in Stage 3 and eleven in Stage 4, each with an observed-failing negative control. **Owner ruled 2026-08-20 (RFC 116, accepted):** the next increment is **negotiation-as-artifacts**, not a network protocol — negotiation is a prerequisite of any protocol, so a protocol without it could only send whole histories. And **this row permits sync-over-any-channel**: its load-bearing clause is *"and both verify it afterward"*, which holds identically however the bytes arrive, so `prikk-store` stays bytes-in/bytes-out and **prikk stays off the network** — every RFC 115 refusal already treats the transport as untrusted, so moving the bytes itself would add a large attack surface and no verification strength. **Stated limit:** confidentiality becomes the user's choice of channel rather than a prikk guarantee (prikk offers none today either). **The architect proposed this reading and disclosed that it also favours the architect's own recommended path before the owner ruled.** **MET, 2026-08-22 (`c9c8576`) — with stated limits; read them before citing this row.** **Delivered by RFC 115 and RFC 116, ten increments, each reviewed and merged at its exact reviewed commit.** A user runs `prikk sync`: `summary` → `compare` → `have` → `build` → `accept` → `pending` → `seal`, over `PSYNCSU1`/`PSYNCHV1` negotiation artifacts and the `PEXCH001` exchange artifact. Patch-level exchange as the owner ruled it (§2.2), with block recognition travelling as a signed claim that carries the block's patch order (D6) and parents (N3), so claims are **topologically ordered for sealing** and **multi-block** histories work. **Evidence, not inference:** `crates/prikk-cli/tests/rfc116_sync_cli.rs` drives the entire loop through the binary alone, asserts by **reading the receiver's own ref tip back**, and — since `83ea31d` — runs `prikk verify` on **both** repositories and asserts both pass, which is this row's load-bearing clause. That assertion has a control: planting a defect in material the sync itself created makes it fire. **Security posture:** trust never expands on receipt (no artifact can adopt a maintainer key); every byte is verified on arrival; the receiver seals only under its **own** key; divergence is reported **as divergence, not corruption**; and both receiving paths — `import_bundle` and `accept_exchange_artifact` — enforce the same closure rule (`b584d0d`). Twenty-one security refusals across the arc, each with an observed-failing control. **Stated limits.** **(a) prikk does not move the bytes** — RFC 116's accepted ruling is that sync-over-any-channel satisfies this row, so the user's existing channel carries the files and **confidentiality is that channel's property, never prikk's**. **(b) Two hosts exchange sealed history, and both verify it** — **corrected 2026-08-26, CI now tests what this row previously said was untested.** The mechanism is file-based and channel-agnostic, so nothing in it is host-local; `ci.yml`'s three `cross-host sync` jobs author on Linux, hand the `PEXCH002` exchange artifact to Windows, and the receiver runs `sync accept`, `sync seal`, and `verify`. **All four sealed-block ids are byte-identical to the sender's**, so the receiver holds the sender's own objects, not re-derived equivalents. Landed `74be0ab`. The artifact still travels as the operator's own channel (limit (a)), and discovery and remote-tracking remain absent (limit (d)) — this closes (b) and nothing else. **(c) Negotiation is branch-scoped; tags travel** — **corrected 2026-08-22, RFC 117 answered what this row previously called an open question.** The summary covers `heads/*` only, and `remotes/*` is excluded structurally, but **tags whose target block lies in the synced ref's ancestry travel in the exchange artifact** (`PEXCH002`) and are adopted by a separate explicit act (`sync adopt-tag`), under the **adopter's own key**. A tag names a **patch set**, not a block — the identity that is stable across repositories — because blocks diverge by design. Merged `faa4d39`. **What remains open for tags:** deletion and movement do not travel, and adoption resolves by scanning local blocks at a cost measured superlinear (12.6 ms over 500 blocks, 86 ms over 2000). **(d) No discovery, remote identity, or remote-tracking semantics** (DC-78 §D4). **The reading was stated before the work, not after** (RFC 111 §8's precedent), including the architect's disclosure that the reading permitting sync-over-any-channel also favoured the architect's own recommended path |
| 2 | **The format-stability question is answered, and its answer honoured** — either canonical encoding is frozen with a stated compatibility promise, or a migration guarantee exists that a repository written by one release is readable by the next | The badge's own sentence says migration may be required. DC-75 added two `BlockPayload` fields on 2026-08-08 | **MET, 2026-08-19** — RFC 114, merged `984c8f1`. **Answered:** verification-bearing bytes are frozen forever (the object-id preimage, each shipped `(object_type, schema_version)` pair's canonical encoding, the signature preimage, the algorithm identifiers); representation may change behind a documented, tested migration path (repository format, containers, index, WAL, bundle). **Honoured by gates, not by intention:** literal identity vectors per shipped pair that fail if the frozen surface moves, a completeness guard so an admitted pair cannot start being written without one (**this row overstated the guard until 2026-08-24**: it was `ObjectType`-granular, so a *second* schema on an already-vectored type passed silently — `RefState` had two schemas in that state since DC-61, and it surfaced only when `(Patch, 2)` was added and the guard stayed quiet. **Fixed pair-granular at `f1528b8`, and observed failing in both directions** — an unvectored pair added, and a vectored pair removed — so the claim is now true by gate rather than by hand. **Residual: the guard sweeps schema versions `0..=8` only**), and a tripwire making a `CURRENT_FORMAT_VERSION` bump **unable to pass CI without migration coverage** — each observed failing before its fix. **Freezing is not "never add a field":** `schema_version` sits inside the id preimage, so a new field is a new version and nothing already written moves. **Scope, per the owner's ruling:** the contract binds **format 6 onward**; formats 1-5 are **not supported** (prikk has never been in production), and their refusal messages now say so rather than offering a migration the product cannot honour. **Residual:** the format-7 carry-forward operation does not yet exist — the tripwire guarantees someone is *stopped*, not that the operation is ready (RFC 114 §5.2) |
...
| 4 | **The signer bootstrap has occurred** — DC-35's authority transaction, approved by two distinct natural persons | Release authority has never been established for any release to date | **OPEN, and deliberately so — not a blocker. Read this row before citing it.** **`release-signers.toml` is empty and fail-closed**, and `docs/src/reference/release-compatibility.md:16` already states no release satisfies the gate, so nothing here is claimed that is not true. **Scope, from DC-35's own "Open-source governance boundary":** this authority governs **only** official upstream prikk tags, release-page assets and package namespaces. It **does not restrict** source contributions, review participation, Apache-2.0 forks, downstream builds, or downstream tags and packages under distinct identities. **No solo developer is disabled by it — including one working on prikk itself**, who may contribute, fork, build and publish under their own identity. **What the two persons are for:** DC-35 defines release authority as a *composition*, not a self-authenticating signer file — "control of an existing signer alone is not authority". The second person exists to prevent **unilateral self-granting of release authority**, so a compromised key cannot enthrone itself. **DC-35 is careful here and the short form of this row previously was not:** multiple operators are *"encouraged when available but not an M1 precondition"* — a single signer may **operate** alone; only the **authority transaction** that grants or changes signing rights needs two people. **Automation may verify evidence but cannot occupy either approval identity**, which excludes the architect as well. **Owner ruling 2026-08-22: the governance is not wrong, it is too early to apply.** prikk does not need official-release status yet; the threat DC-35 guards against becomes real only once people depend on official releases, and today nobody does. **Revisit when either occurs, whichever is first: a second natural person joins the project, or prikk claims production readiness.** Until then this row reports a status, not an obstacle — **it blocks the official-release claim, not the software, and not this project's work.** A local rehearsal may exercise the mechanism (transaction/proof/challenge records, fail-closed behaviour) and **cannot** satisfy this row, because processes supply no independent judgment |
...
| 6 | **Mutation works wherever the project claims support** | A tool that can read but not write on two of three supported platforms is not past "early" | **MET, 0.21.0 (2026-08-16).** Linux, macOS, and Windows all mutate; the suite runs on all three in CI, and a repository authored on Linux, mutated on Windows, and verified on Linux is required to produce byte-identical object ids. Windows carries **two** documented narrower guarantees (`platform-support.md`) — named, not silent. **Corrected 2026-08-23:** this row said *four* through 0.21.0, which was right then; **0.22.0 closed two of them** (`prikk unlock` reports real process liveness on Windows, and anchor identity uses the 128-bit file identifier) and replaced argument with evidence for the rest. The remaining two are still named, including the resolution race Windows cannot close by construction — it has no `openat` equivalent |
