...
| **`checkout --patch-materialize` cannot replay `ReplaceBinary` or `ChangePerm`** — `patch_replay/decode.rs:249` names only `CreateFile`, file-`DeleteNode`, and `EditText` as wired | **Capability gap, pre-existing and partially known.** Surfaced 2026-08-02 by DC-67 sequences 2 and 5 as the first place it blocks *ordinary* verification: content-level rebuild cannot be asserted for binary edits or mode changes | Unowned. The code describes it as awaiting the node model, so this is a design question, not a missing match arm | Product **M3** |
