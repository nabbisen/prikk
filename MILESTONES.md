...
| `TagPayload.created_at` documented as an "authoritative creation timestamp" while DC-34 ruled there is no clock authority and zeroed the analogous `RefUpdate` field | Documentation/design contradiction, surfaced 2026-07-30 by DC-63 | DC-63 must resolve; recommendation is to write the no-clock sentinel and correct the doc comment | Product **M1** |
