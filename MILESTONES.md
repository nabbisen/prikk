...
|---|---|---|---|
| 1 | **Sync exists** — two machines can exchange sealed history, and both verify it afterward | A *distributed* VCS that cannot distribute. **Corrected 2026-08-19 by RFC 115's investigation: the second half of this row was wrong.** Exchange between repositories does exist — `bundle export`/`import` move a genesis-complete closure, imported history lands in the `remotes/` received namespace without advancing any local ref (DC-78, by design), `merge` accepts a received ref as one side (DC-85), and since DC-53 Stage 2 a receiver cryptographically verifies imported authorship. **All of it is tested end to end** | **OPEN. RFC 115 Stages 1-3 shipped 2026-08-20 (`0128c91`); two named gaps remain.** **Delivered:** patch-level exchange as the owner ruled it (§2.2) — the patch-set digest (`cea29f4`), the signed recognition claim carrying block-level recognition without dragging lineage (`106243e`), and the `PEXCH001` exchange artifact with its accept path (`93429dc`), plus a received-namespace `verify` stage that closes the gap where a dangling received ref was invisible on both sides. Ten security refusals, each with an observed-failing negative control. **Trust does not expand on receipt: no artifact can cause a maintainer key to be adopted.** **Still missing, and both are required by this row as written:** **(a) sealing what you accepted** — `seal` builds blocks from the active WAL and an accepted patch was never in the WAL, so a receiver can accept, verify and store patches but cannot yet turn them into sealed history. Under the owner's patch-unit ruling the receiver seals locally, so **this row cannot be met without it**; scheduled as **RFC 115 Stage 4**, ahead of transport (owner, 2026-08-20). **(b) transport** — there is still no network code in the workspace; four options surveyed (§3), none chosen. **Reading stated before the work, not after** (RFC 111 §8's precedent): "exchange sealed history" is read to require that the *receiver* ends holding sealed history, which accept alone does not produce. **§5.1's test and security discipline is binding on the remaining work** |
