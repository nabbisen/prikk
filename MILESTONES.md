...
| `prikk init <path>` fails at its first command when `<path>` does not exist, instead of creating it — `RepositoryLayout::init` opens the worktree root via `MutationRoot::open` (`AnchoredDirectory::open`, no `O_CREAT`) before `.prikk/` creation is ever reached, so a nonexistent target directory returns `ENOENT` straight back to the CLI | **Install-path defect** — found 2026-08-03 running the README's own install sequence against a fresh path, per DC-70's handoff standing request ("run what you publish"). The Quick Start's `mkdir -p ./sample-repo && cd` masks it by never exercising the nonexistent-path case | Unowned. Reported, not fixed, per the handoff's explicit instruction | Corrective M2 |
