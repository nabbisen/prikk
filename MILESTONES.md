...
| Commit loads the entire worktree into memory — `WorktreeFile { bytes: Vec<u8> }` per file, O(total worktree bytes) regardless of change size | **Scalability defect, previously untracked and unnamed by any requirement** — recorded 2026-07-30 from the DC-56 design review. NFR-PERF-01 bounds latency only | DC-56 (in scope alongside the latency objective; the same index fixes both). The memory axis it evidences against is DC-62, complete at `07b1fc8` | Product **M1**, alongside NFR-PERF-01 |
