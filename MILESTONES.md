...
| DC-62's memory table publishes absolute `VmHWM` with no baseline row, so the content-proportional component is present but not visible — 1,000→10,000 files reads as 2.58x growth where above the process floor it is 9.84x | **Presentation, not data.** Matters because DC-56's criterion 5 evidences its memory improvement against this table, and without a baseline it cannot distinguish eliminating the full-tree read from merely reducing it | Add a baseline reference row or delta column to the memory section. Recorded 2026-07-30 from the DC-62 review (N1); should land before DC-56's implementation review | Corrective M2 |
