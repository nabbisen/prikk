...
| M3 | Block DAG and Checkout | **partially** — checkout and block DAG shipped; **multi-patch active blocks now implemented** (DC-66), making NFR-PERF-02/03 reachable, but DC-57's actual threshold values are not yet set | Migration and recoverable backup |
...
| Active-Patch warning 800 / hard bound 1000 from NFR-PERF-02 | **Missed product gate** — the capability gap that made it unreachable is resolved (DC-66); the threshold values themselves are still unset | DC-57 **HELD**, now blocked on choosing and implementing the actual 800/1000 thresholds, not on capability | Product **M3**; carried into corrective M2 |
| Merge scope bounded by active block size (NFR-PERF-03) | **Was vacuously satisfied; now reachable** — DC-66 makes a bounded active block a real state, not an unreachable one | DC-57's scope once thresholds are set | Product **M3** |
