...
| Multi-patch active blocks not implemented — active WAL capped at one record | **Capability gap; blocks two NFRs** | **DECIDED 2026-08-02 by the project owner: multi-commit queuing is a scheduled capability** (`rfcs/handoffs/DC-57-active-patch-thresholds/queuing-decision-record-v1.md`). NFR-PERF-02/03 need no amendment; both become meetable. **DC-57 stays held**, now blocked on the queuing increment rather than on a decision. Measurement done 2026-08-02: **batching does not amortize DC-64's residual cost** — `load`/`persist`/`from_replay` are per-`commit`-invocation, and batching removes seals, not commits. Batching saves seal cost only. **No measurement blocks the queuing increment**; DC-64's residual finding stays open and unowned | Product **M3** |
