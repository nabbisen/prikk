...
| 5 | **`verify` checks author signatures repository-wide** — **Corrected 2026-08-09 by DC-78's investigation: NOT a prerequisite of criterion 1.** Exchange is ruled to claim only "sealed by a Maintainer key you adopted", which `trust.rs` can already verify — so a receiver verifies exactly as much as a local user does, and criterion 1 is reachable without this one. **But shipping exchange makes this criterion more important, not less**, since other people's history then arrives with authorship unchecked | The positioning is that every change is signed by its author *and verifiable by anyone*. **Half delivered 2026-08-18.** `verify` now cryptographically checks every reachable Patch's AUTHOR signature against recorded key material (DC-53 Stage 1, merged `970bc27`): a signature that fails against recorded material fails `verify`'s exit status, and a Patch whose key material was never recorded is reported *unverifiable* rather than sound. The earlier statement here — that the only cryptographic verification in the product is one policy-signature call site — is no longer true | **OPEN, narrowed to "by anyone".** AUTHOR key material is recorded at authoring time and **does not travel**: `bundle.rs` carries no author key container, so a Patch received from another party is permanently unverifiable on the receiver's side — the exact case this criterion's own note warns of. **Closing it requires DC-53 Stage 2 to deliver transport *and* pinning together**; pinning without transport verifies nothing for a receiver, transport without pinning verifies nothing at all (`rfcs/handoffs/DC-53-repository-wide-author-trust-verification/design-v1.md` §5, v1.2). Scheduled after criterion 3 by the owner, 2026-08-18 |
