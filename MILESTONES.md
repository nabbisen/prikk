...
| `prikk-store` does not compile at all off Linux — `crates/prikk-store/src/fsutil/anchored/{immutable,regular,read}.rs` carry no `#[cfg(target_os = "linux")]` gate but unconditionally import helpers (`open_existing_regular_if_exists`, `open_new_regular`, `io_error`, `prepare_directory_required`) that are themselves gated to Linux at their definition sites | **Contradicts DC-37's stated design** — its mutation support matrix frames macOS/Windows as "read-only/diagnostic use only," which presumes the binary builds there. It does not: `cargo build -p prikk --release --target x86_64-pc-windows-gnu --locked` fails with four `E0432` unresolved-import errors, not a runtime `unsupported_mutation()` refusal. Found 2026-08-03 by DC-70's required trial build ("verify by building, do not assume"), reported not fixed — DC-37/fsutil territory, not a release-CI increment | Unowned. Fixing it means auditing and correctly re-gating three fsutil files for non-Linux stub behavior mirroring `directory.rs`'s existing pattern | Product **M1**, alongside DC-37 |
