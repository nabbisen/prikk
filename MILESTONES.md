...
| 5 | **`verify` checks author signatures repository-wide** — **and see criterion 1: this is likely a *prerequisite* of exchange rather than independent of it, since a receiver cannot meaningfully check what it was sent while nothing checks author signatures (recorded 2026-08-09, to be confirmed by DC-78's investigation)** | The positioning is that every change is signed by its author *and verifiable by anyone*. Signatures are carried and preserved, but `verify` never checks them — the only cryptographic verification in the product is one policy-signature call site | **DC-53, proposed and unscheduled** |
