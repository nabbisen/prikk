...
//! rejects a malformed-shape signature under `RepositoryFormat::CurrentV5` before
...
pub struct RepositoryVerification {
    ...
    /// Phase B: one outcome per `CurrentV5` Block whose Phase A check succeeded, in the
    /// state-dependency order `verify_blocks_topological` resolved them — not scan order (DC-92
    /// §4.2). Empty when the `Objects` stage did not evaluate, or when no `CurrentV5` Block passed
    /// Phase A at all.
...
impl RepositoryVerification {
    ...
    /// Phase A object whose own check failed, a Phase B `CurrentV5` Block whose state-root check
...
/// the merge-baseline re-derivation. A `CurrentV5` block's own state-root verification is
...
fn verify_block_payload(
    ...
    let merge_baseline_divergence = if format == RepositoryFormat::CurrentV5 {
        verify_merge_baseline(object_store, block_id, &payload)?
    } else {
        None
    };
    if format == RepositoryFormat::CurrentV5 {
