...
fn publish_snapshot_then_patch_block(layout: &RepositoryLayout) -> prikk_error::Result<()> {
    ...
    let root_ref_update = signed_ref_update_envelope("heads/main", None, root_ref_state_id, root_block_id, 1);
    ...
    let patch_ref_state = signed_ref_state_envelope("heads/main", Some(root_ref_state_id), patch_block_id, 2);
...
fn write_blob(store: &mut FileObjectStore, bytes: &[u8]) -> prikk_error::Result<prikk_object::ObjectId> {
...
fn signed_block(
    ...
    let mut envelope = ObjectEnvelope::unsigned(ObjectType::Block, 1, payload_bytes.unwrap_or_default());
