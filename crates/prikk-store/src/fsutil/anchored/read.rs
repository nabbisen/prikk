...
/// Bytes and raw mode read from one validated regular-file handle.
...
/// Read a regular file, returning `None` only when a path component is absent.
pub(crate) fn read_file_if_exists(root: &MutationRoot, relative: &Path) -> Result<Option<Vec<u8>>> {
    read_file_state_if_exists(root, relative).map(|file| file.map(|file| file.bytes))
}

/// Read bytes and mode from one validated regular-file handle.
pub(crate) fn read_file_state_if_exists(
    root: &MutationRoot,
    relative: &Path,
) -> Result<Option<RootFile>> {
    #[cfg(target_os = "linux")]
    {
        ...
        match std::fs::read(&path) {
            Ok(bytes) => {
                ...
                let mode = {
                    ...
                    std::fs::metadata(&path)?.mode()
                };
                #[cfg(not(unix))]
                let mode = 0;
                Ok(Some(RootFile { bytes, mode }))
            }
