...
/// `validate_read_schema` under `RepositoryFormat::CurrentV4` calls `envelope.validate_strict()`,
...
/// checked only `schema_version`, never calling `validate_strict()` -- `CurrentV4` is the only format
