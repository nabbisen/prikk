...
fn require_rollback_author_signature<'a>(
    ...
    envelope
        ...
        .and_then(|signature| {
            ...
            // record before this function ever runs, and under `RepositoryFormat::CurrentV4` that
            // call already hard-errors on a malformed-length signature via `envelope.validate_strict()`.
            // With format-1 and format-2 both retired (RFC 102 Stage 3), `CurrentV4` is the only
